...
Go = {
    ...
    send_move: function(move_data) {
        ...
        $.postJSON(Go.game.moves.url(), move_data,
                   function(response) {
                       // FIXME: Do something with the response.
                       // The stone could be drawn directly when the
                       // local player makes a move, and then removed
                       // if the move turns out to be bad. Of course,
                       // some of this logic should be in the client,
                       // so that obvious bad moves aren't even
                       // possible.
...
Go.Game = Backbone.Model.extend({
    ...
    add_captures:  function (move, index, moves) {
        ...
        if (captured) {
            console.log("adding captures:", captured);
...
Go.BoardView = Backbone.View.extend({
    ...
    show_move: function(move) {
        console.log("show_move: ", move.get("n"), move.get("position"), move);
...
Go.ChatView = Backbone.View.extend({
    ...
    show_message: function(message, index, messages, noscroll) {
        ...
        console.log("displaying message " + message.get("id"));

        // TODO: Do this with some templating magic instead
        var node = $("<div>");
        node.attr("id", "m" + message.get("id"))
        node.addClass("message");
        node.text(message.get("user") + ": " + message.get("body"));
        //node.hide();
        $("#inbox").append(node);
        if (!noscroll) {
